#ifndef ADK_IMPL_HTTP_SERVER_H_
#define ADK_IMPL_HTTP_SERVER_H_
...
// Add by liyc, use boost::regex to avoid gcc version difference
...
namespace http
{
    ...
    public:
        ...
        public:
            ...
            std::map<regex_orderable, std::set<std::string>> verification_filter_resource;
    ...
    public:
        // method invoked before the http server resource_function
        std::function<void(const std::string& method, const std::string& path)> request_hook_before;
        // method invoked after the http server resource_function
        std::function<void(const std::string& method, const std::string& path)> request_hook_after;
        std::mutex mutex_;
        ...
        virtual void start() {
            ...
            acceptor->listen(config.backlog);
        ...
        void stop() {
            if (acceptor)
                acceptor->close();
            if(config.thread_pool_size>0 && io_service)
                io_service->stop();
        }
    ...
    protected:
        ...
        std::shared_ptr<boost::asio::steady_timer> get_timeout_timer(const std::shared_ptr<socket_type> &socket, long seconds) {
            if(seconds==0)
                return nullptr;
        ...
        void find_resource(const std::shared_ptr<socket_type> &socket, const std::shared_ptr<Request> &request) {
            ...
            {
                std::lock_guard<std::mutex> lck(config.mtx);
                // verification password
                if (!config.read_password.empty()
                    || !config.write_password.empty())
                {
                    REGEX_NS::regex reg("^/|((.)+\\.(jpg|png|js|css|ico|md|svg|ttf|eot|woff))$");
                    if (!REGEX_NS::regex_match(request->path, reg)
                        && !is_filter_resource(request))
                    {
                        auto ds_it = request->header.find("AMIDomainServerClient");
                        if (ds_it == request->header.end()
                            || ds_it->second != "__AMIDomainServerClient__")
                        {
                            auto it = request->header.find("AMIDSAuthority");
                            if (it == request->header.end())
                            {
                                write_unauthorized_response(socket, request);
                                return;
                            }

                            // read password only for http get
                            if (it->second == config.read_password)
                            {
                            ...
                            else if (it->second != config.write_password) // write password
                            {
                                write_unauthorized_response(socket, request);
                                return;
                            }   
        ...
        void write_response(const std::shared_ptr<socket_type> &socket, const std::shared_ptr<Request> &request, 
            ...
            try {
                if (request_hook_before)
                {
                    request_hook_before(request->method, request->path);
                }

                resource_function(response, request);

                if (request_hook_after)
                {
                    request_hook_after(request->method, request->path);
                }
            }
            catch(const std::exception &e) {
        ...
        bool is_filter_resource(const std::shared_ptr<Request> &request)
        {
            for(auto &regex_method : config.verification_filter_resource) 
            {
                auto it = regex_method.second.find(request->method);
                if(it != regex_method.second.end())
                {
                    if( REGEX_NS::regex_match(request->path, regex_method.first)) 
                    {
                        return true;
                    }
                }
        ...
        void write_unauthorized_response(const std::shared_ptr<socket_type> &socket,
                                         const std::shared_ptr<Request> &request)
        {
            try
            {
                ...
                auto timer = this->get_timeout_timer(socket, config.timeout_content);
                // new Response
                auto response = std::shared_ptr<Response>(new Response(socket), [](Response *response_ptr) { delete response_ptr; });
                ...
                *response << "HTTP/1.1 401 Unauthorized\r\n"
                ...
                // send response
                this->send(response, [this, request, response, timer](const boost::system::error_code &ec) {
                    if (timer)
                        timer->cancel();
                    if (on_error)
                        on_error(request, ec);
