 ...
 * @date 2017-03-22
 */
#ifndef ADK_IMPL_RESPONSE_BUILDER_H_
#define ADK_IMPL_RESPONSE_BUILDER_H_
...
using DescMap = std::map<uint16_t, std::string>;

/// 常用HTTP状态码到描述信息的映射表
static DescMap* CreateDescMap()
{
    // clang-format off
    DescMap* desc_map = new DescMap();
    *desc_map = boost::assign::map_list_of
        (kContinue, "Continue")
    ...
    .convert_to_container<status_code::DescMap>();
    // clang-format on
    return desc_map;
}
...
{
    static DescMap* s_msg_desc_ptr = CreateDescMap();
    auto it = s_msg_desc_ptr->find(status);
    return (it != s_msg_desc_ptr->end()) ? it->second : std::string();
}
