...
#ifndef ADK_HTTPREQUEST_HPP
#define ADK_HTTPREQUEST_HPP
...
namespace http
{
    ...
    inline namespace detail
    {
        ...
        private:
            ...
            {
#ifdef  _WIN32
                WSAPOLLFD endpoint_event[1];
                endpoint_event[0].fd = endpoint;
                switch (type)
                {
                case SelectType::read:
                    endpoint_event[0].events = POLLRDNORM | POLLRDBAND;
                    break;
                case SelectType::write:
                    endpoint_event[0].events = POLLWRNORM;
                    break;
                default:
                    break;
                }
#else
                struct pollfd endpoint_event[1];
                endpoint_event[0].fd = endpoint;
                switch (type)
                {
                case SelectType::read:
                    endpoint_event[0].events = POLLIN | POLLPRI;
                    break;
                case SelectType::write:
                    endpoint_event[0].events = POLLOUT;
                    break;
                default:
                    break;
                }
...
#ifdef _WIN32
                auto count = ::WSAPoll(endpoint_event, 1, timeout);
                while (count == -1 && WSAGetLastError() == WSAEINTR)
                {
                    count = ::WSAPoll(endpoint_event, 1, timeout);
                }
...
#else
                auto count = ::poll(endpoint_event, 1, timeout);
                while (count == -1 && errno == EINTR)
                {
                    count = ::poll(endpoint_event, 1, timeout);
                }
...
} // namespace access_agent
#endif // ADK_HTTPREQUEST_HPP
