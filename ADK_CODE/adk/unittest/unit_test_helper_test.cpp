...
BOOST_AUTO_TEST_CASE(test_mockable)
{
    ...
    BOOST_CHECK_EQUAL(base_ptr->Foo(), "world");
}
...
BOOST_AUTO_TEST_CASE(test_mockable_return)
{
    ...
    ret_orig = false;
    BOOST_CHECK(test.ExeTestV2() != (&test));
    ...
    test.ExeTestV3(ret_output);
    BOOST_CHECK_EQUAL(ret_output, false);
    BOOST_CHECK_EQUAL(ret_output, false);
}
