#include <adk_pack/thread_pool.h>
...
MPSCQueue* g_restart_channel;
class MyRestartHandler
{
public:
    void DoRestart(const adk::ThreadPool::DispatcherType& dispatcher)
    {
        g_restart_channel->Push((void*)&dispatcher);
    }
};

void DrivenThread()
{
    std::cout << "Driven thread id " << boost::this_thread::get_id() << std::endl;
    void* dispatcher_ptr;
    while (is_running)
    {
        if (g_restart_channel->Pop(dispatcher_ptr) == ErrorCode::kSuccess)
        {
            (*(const adk::ThreadPool::DispatcherType*)dispatcher_ptr)();
        }
...
{
    {
        std::cout << "test case, no new thread lauched" << std::endl;
        ThreadPool thr_pool;
        thr_pool.set_nr_init_thr(1);
        thr_pool.set_nr_max_thr(2);
    ...
    {
        std::cout << "test case, lauched thread" << std::endl;
        ThreadPool thr_pool;
        thr_pool.set_nr_init_thr(1);
        thr_pool.set_nr_max_thr(2);
        thr_pool.set_nr_max_cc_jobs(4096);
    ...
    {
        std::cout << "test case, dispatcher was driven by 3rd thread" << std::endl;
        ThreadPool thr_pool;
        thr_pool.set_nr_init_thr(1);
        thr_pool.set_nr_max_thr(2);
        thr_pool.set_nr_max_cc_jobs(4096);
        ...
        g_restart_channel = MPSCQueue::Create("driven_thread", sizeof(void*), 256);
        boost::thread driven_thread = boost::thread(boost::bind(DrivenThread));
        MyRestartHandler my_handler;
        thr_pool.Start(boost::bind(&MyRestartHandler::DoRestart, &my_handler, _1));
