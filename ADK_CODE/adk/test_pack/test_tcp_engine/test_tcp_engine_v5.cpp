...
int main(int argc, char* argv[])
{
    ...
    const auto heartbeat_timeout1 = 3600 * 3;
    const auto heartbeat_timeout2 = 5;

    Property client_props;
    client_props(adk::io_engine::config::endpoint::kHeartbeatTimeoutMilli, heartbeat_timeout1 * 1000)
    ...
    std::cout << "heartbeat timeout is " << heartbeat_timeout1 << std::endl;
    sleep(10 * heartbeat_timeout1);

    Property update_prop;
    update_prop(adk::io_engine::config::endpoint::kHeartbeatTimeoutMilli, 5000);
    endpoint->UpdateProperty(update_prop);

    std::cout << "heartbeat timeout is " << heartbeat_timeout2 << std::endl;
    sleep(10 * heartbeat_timeout2);
