...
#define ADK_IS_OOB_MSG(msg_ptr) (((adk::ThreadMessageBase<void*>*)msg_ptr)->message_type() < 0)
...
{
    int32_t ret = adk_impl::atomic_inc(is_oob ? g_oob_msg_type_current : g_msg_type_current) 
                  * (is_oob ? -1 : 1);
...
void ThreadGcBase::DoGC()
{
#ifdef __ADK_THREAD_TEST__
    static uint32_t gc_counter = 0;
    ++gc_counter;
    std::cout << message_type_name_ << " gc_counter = " << gc_counter << " "
        << this << std::endl;
#endif

    assert(ref_counter_ == 0);
    int32_t ec = this->Delete();
    ADK_NOTUSE(ec);
    assert(ec == ErrorCode::kSuccess);
}

void ThreadGcBase::hold_reference()
{
    if (ref_counter_ <= 1)
        ++ref_counter_;
    else
        adk_impl::atomic_inc(ref_counter_);
}

void ThreadGcBase::put_reference()
{
    if (ref_counter_ == 1)
    {
        ref_counter_ = 0;
    ...
    {
        if (adk_impl::atomic_dec(ref_counter_) != 1)
        {
            return;
        }
    }
...
bool ThreadGcBase::drop_reference()
{
    if (ref_counter_ == 1)
    {
        ref_counter_ = 0;
...
{
    Reset();
    number_create_instance = 0;
    is_default = true;
}

void ThreadParams::Reset()
{
    INIT_PARAM(msg_budget, ADK_THREAD_MESSAGES_BUDGET);
    INIT_PARAM(oob_msg_budget, ADK_THREAD_OOB_MESSAGES_BUDGET);
    INIT_PARAM(mq_cont_process_limit,  ADK_THREAD_MQ_CONT_PROCESS_LIMIT);
    INIT_PARAM(number_instance, 1);
    INIT_PARAM(init_priority, 1);
    INIT_PARAM(busy_poll_ns, 100000);
    INIT_PARAM(wait_timeout_ns, 100000);
    INIT_PARAM(backoff_limit, 64);
    INIT_PARAM(event_mode, adk::thread::kInterrupt);
    INIT_PARAM(parallel_init, false);
    INIT_PARAM(thread_affinity, "");
}

ThreadParams& ThreadParams::operator= (const ThreadParams& other)
{
    COPY_PARAM(msg_budget);
    COPY_PARAM(oob_msg_budget);
    COPY_PARAM(mq_cont_process_limit);
    COPY_PARAM(number_instance);
    COPY_PARAM(event_mode);
    COPY_PARAM(init_priority);
    COPY_PARAM(busy_poll_ns);
    COPY_PARAM(wait_timeout_ns);
    COPY_PARAM(backoff_limit);
    COPY_PARAM(parallel_init);
    COPY_PARAM(thread_affinity);
    return *this;
}
...
ThreadBase* ThreadManager::ThreadInstance(int32_t thr_tag, int32_t instance_id)
{
    return threads_[thr_tag * ADK_MAX_THREAD_INSTANCE + instance_id];
}
...
int32_t ThreadManager::AddThread(ThreadBase* app_thread)
{
    ...
    threads_[app_thread->thread_tag_ * ADK_MAX_THREAD_INSTANCE + app_thread->instance_id_] = app_thread;
...
{
    threads_ = new ThreadBase*[ADK_MAX_THREAD_CLASS * ADK_MAX_THREAD_INSTANCE];
    memset(threads_, 0x00, sizeof(ThreadBase*) * ADK_MAX_THREAD_CLASS * ADK_MAX_THREAD_INSTANCE);
    nr_threads_ = 0;
    memset(threads_params_, 0x00, sizeof(threads_params_));
    is_init_ = false;
}
...
void ThreadBase::Accept(void* msg_ptr)
{
    ...
    reinterpret_cast<adk_impl::SimpleEventManager*>(ev_mana_)->Notify([this, msg_ptr]() {
        return reinterpret_cast<adk_impl::MPSCQueue*>(thread_message_queue_[ADK_IS_OOB_MSG(msg_ptr)])->Push(msg_ptr);
    });
}
...
int32_t ThreadBase::AcceptNoBlock(void* msg_ptr)
{
    ...
    auto ret = reinterpret_cast<adk_impl::MPSCQueue *>(thread_message_queue_[ADK_IS_OOB_MSG(msg_ptr)])->Push(msg_ptr);
    if(ret == ErrorCode::kSuccess)
        reinterpret_cast<adk_impl::SimpleEventManager *>(ev_mana_)->Notify([]() { return ErrorCode::kSuccess; });
    return ret;
}
...
int32_t ThreadBase::PollOOBMessage()
{
    ...
    {
        if (reinterpret_cast<adk_impl::MPSCQueue*>(thread_message_queue_[ADK_OOB_MQ])->Pop(msg_ptr_)
            == ErrorCode::kSuccess)
        {
            ++oob_mq_srv_cnt_;
            DeliverOOBMessage();
...
void ThreadBase::DoAllocCacheQueue(int32_t tag)
{
    ...
    cache_queues_[tag].cache_queue = 
            (void*)adk_impl::SPSCUnboundedQueue<void*>::Create(cache_queues_[tag].queue_name);
}
...
void ThreadBase::DeliverMessage()
{
    ...
    if (ADK_UNLIKELY(cache_map_[tag]))
    {
        ...
        reinterpret_cast<adk_impl::SPSCUnboundedQueue<void*>*>(cache_queues_[tag].cache_queue)->Push(msg_ptr_);
...
void ThreadBase::DeliverCachedMessage()
{
    ...
    assert(cache_queues_[tag].cache_queue != NULL);
    reinterpret_cast<adk_impl::SPSCUnboundedQueue<void*>*>(cache_queues_[tag].cache_queue)->Pop();
...
bool ThreadBase::PopCachedMessage()
{
    ...
    {
        reinterpret_cast<adk_impl::SPSCQueue<void**>*>(release_queue_)->Pop(current_cache_queue_);
        nr_cq_srv_cnt_ = 0;
    }
    
    ++nr_cq_srv_cnt_; 
    void** head_ptr = (*reinterpret_cast<adk_impl::SPSCUnboundedQueue<void*>**>(current_cache_queue_))->Head();
    if (head_ptr != NULL)
    {
        msg_ptr_ = *head_ptr;
        if (nr_cq_srv_cnt_ >= nr_cq_srv_budget_)
        {
            reinterpret_cast<adk_impl::SPSCQueue<void**>*>(release_queue_)->Push(current_cache_queue_);
...
{ 
    ...
    if (block_map_[tag])
    {
        ...
        {
            reinterpret_cast<adk_impl::SPSCQueue<void**>*>(release_queue_)->Push(&(cache_queues_[tag].cache_queue));
            ++nr_release_cache_queues_;
...
{ 
    ...
    if (cache_queues_[tag].release_alert)
    {
        ...
        while (reinterpret_cast<adk_impl::SPSCQueue<void**>*>(release_queue_)->Pop(cache_queue) == ErrorCode::kSuccess)
        {
            ...
            reinterpret_cast<adk_impl::SPSCQueue<void**>*>(release_queue_)->Push(cache_queue);
        }
...
{
    adk_impl::ScopedSpinlock mana_lock_guard(timer_mana_lock_);
...
{
    auto time_value_now = adk_impl::timespec_now();
    TimerIdType index = AllocTimerIndex();
    if (index == -1u)
        return kInvalidTimerHandler;

    auto& timer = timers_[index];
    adk_impl::ScopedSpinlock timer_lock_guard(timer.lock_);
                        ...
                        bool          fail_on_timeout)
{
    ...
    else
    {
                 ...
                 || timer.state_ == TimerState::kCancelled)
        {
            timer.interval_ = duration;
            timer.state_ = TimerState::kRunning;
            timer.expire_time_ = adk_impl::timespec_now() + duration;
            timer.adjust_expire_time_ = 0;
            SchedTimerToRun(timer.expire_time_, hdl);
        }
        else 
...
{
    auto time_val_now = adk_impl::timespec_now();
    {
        adk_impl::ScopedSpinlock mana_lock_guard(timer_mana_lock_);
        nr_expire_timers_ = 0;
    ...
    time_val_now = adk_impl::timespec_now();
    i = -1;
    do {
        if ((++i) >= nr_expire_timers_)
            break;
...
{
    int64_t run_begin = adk_impl::timespec_now();
    uint64_t counter = 0;
    while (is_running_)
    {
        ++counter;
        int64_t time_to_sleep = run_begin + (1000000 * counter) - adk_impl::timespec_now();
        if (time_to_sleep > 0)
...
{
    adk_impl::ScopedSpinlock join_lock_guard(join_lock_);
    {
        adk_impl::ScopedSpinlock mana_lock_guard(timer_mana_lock_);
        if (is_running_)
            return;
    ...
    timer_thread_ = boost::thread(&ThreadTimerManager::TimerThreadMain, this);
}
...
ThreadTimerManager::ThreadTimerManager()
{
    last_timer_index_ = 0;
    nr_expire_timers_ = 0;
    adk_impl::SpinlockInit(timer_mana_lock_);
    adk_impl::SpinlockInit(join_lock_);
}
...
{
    adk_impl::SetCpuAffinity(thread_params_->thread_affinity);
    ...
    try 
    {
        ...
        while (is_running_)
        {
            if (reinterpret_cast<adk_impl::SimpleEventManager*>(ev_mana_)->Wait(
                [this, &mq_cont_deal_cnt](){
                    ...
                    if ((mq_cont_deal_cnt < static_cast<uint32_t>(mq_cont_process_limit_))
                        && (reinterpret_cast<adk_impl::MPSCQueue*>(thread_message_queue_[ADK_MQ])->Pop(msg_ptr_)
                        == ErrorCode::kSuccess))
                    {
                        DeliverMessage();
                        ++mq_cont_deal_cnt;
                    ...
                    msg_ptr_ = NULL;
                    return adk_impl::ErrorCode::kWouldblock;
                },
...
{
    if (reinterpret_cast<adk_impl::MPSCQueue*>(thread_message_queue_[ADK_MQ])->Pop(msg_ptr_) != ErrorCode::kSuccess)
        usleep(1);

    reinterpret_cast<adk_impl::MPSCQueue*>(thread_message_queue_[ADK_OOB_MQ])->Pop(msg_ptr_);
}
...
void ThreadBase::Dump(boost::property_tree::ptree& ptree)
{
    ...
    reinterpret_cast<adk_impl::MPSCQueue*>(thread_message_queue_[ADK_MQ])->Dump(ptree);
}
...
std::string ThreadBase::Dump(bool is_pretty)
{
    ...
    Dump(ptree_tmp);
    return adk_impl::PtreeToString(ptree_tmp, is_pretty);
}
        ...
        [this, is_in_running_thread](){
            {
                ...
                if (!(thread_li_->common->is_invoke))
                {
                    ...
                    adk_impl::atomic_inc(thread_li_->common->ref_counter);
...
void ThreadBase::Start()
{
    ...
    is_running_ = true;
    thread_ = adk_impl::boost_thread(
                thread_name_,
                thread_name_,
                boost::bind(&ThreadBase::ThreadMain, this));  
}
...
void ThreadBase::Exit(bool is_in_running_thread)
{
    HandleExceptions(
        [this](){
            ...
            {
                boost::mutex::scoped_lock lock_guard(thread_li_->common->lock);
                if (adk_impl::atomic_dec(thread_li_->common->ref_counter) == 1
...
void ThreadBase::Finish()
{
    ...
    {
        is_running_ = false;
        reinterpret_cast<adk_impl::SimpleEventManager*>(ev_mana_)->ReleaseWaitThread();   // FIXME: add reset method
        thread_.join();
    }
...
ThreadTimer::ThreadTimer() 
    : is_used_(false),
      version_(0),
      id_(kInvalidTimerId),
      state_(TimerState::kInit),
      type_(TimerType::kPeriod),
      interval_(0),
      expire_time_(0),
      adjust_expire_time_(0),
      run_timer_thr_(NULL)
{
    adk_impl::SpinlockInit(lock_);
}

void* ThreadUtil::CreateMPSCQueue(const std::string& name, uint32_t entry_payload_size, uint32_t queue_size)
{
    return adk_impl::MPSCQueue::Create(name, entry_payload_size, queue_size);
}

void* ThreadUtil::CreateSPSCQueue(const string& name, uint32_t entry_payload_size)
{
    return adk_impl::SPSCQueue<void*>::Create(name, entry_payload_size);
}

void* ThreadUtil::CreateEventManage(int32_t busy_poll_ns, int32_t backoff_limit)
{
    return new adk_impl::SimpleEventManager(busy_poll_ns, backoff_limit);
}
