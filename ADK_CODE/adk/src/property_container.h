 ...
 * @date 2016-11-28
 */
#ifndef ADK_IMPL_PROPERTY_CONTAINER_H_
#define ADK_IMPL_PROPERTY_CONTAINER_H_
...
public:
    ...
    inline void SetValue(const std::string& key, const Pointer& value);
    ...
    inline void SetValue(const std::string& key, const std::vector<Pointer>& value);
    ...
    inline Pointer GetPointerValue(const std::string& key) const;
    ...
    inline std::vector<Pointer> GetPointerVectorValue(const std::string& key) const;
    ...
    inline Pointer GetValue(
            const std::string& key,
            const Pointer& default_value) const;
    ...
    inline std::vector<Pointer> GetValue(
            const std::string& key,
    ...
    inline void MergeFrom(const PropertyContainer &reference);
...
void PropertyContainer::SetValue(const std::string& key, const Pointer& value)
{
...
void PropertyContainer::SetValue(
        const std::string& key,
        const std::vector<Pointer>& value)
{
    using boost::property_tree::ptree;
    ptree& node = ptree_.put_child(key, ptree());
    for (auto it = value.begin(); it != value.end(); ++it)
...
Pointer PropertyContainer::GetPointerValue(const std::string& key) const
{
    try
    {
        Pointer::LONG_TYPE ptr_value = ptree_.get<Pointer::LONG_TYPE>(key);
        return ptr_value;
    }
    catch (boost::property_tree::ptree_bad_path&)
    {
...
std::vector<Pointer> PropertyContainer::GetPointerVectorValue(const std::string& key) const
{
    try
    {
        using boost::property_tree::ptree;
...
Pointer PropertyContainer::GetValue(
        const std::string& key,
        const Pointer& default_value) const
{
    try
...
std::vector<Pointer> PropertyContainer::GetValue(
        const std::string& key,
        const std::vector<Pointer>& default_value) const
{
    try
...
{
    for (const auto& item : reference.ptree_)
    {
        ptree_.put_child(item.first, item.second);
    }
}

void PropertyContainer::MergeFrom(const PropertyContainer &reference)
{
    for (const auto& item : reference.ptree_)
    {
        if (!HasValue(item.first))
        {
            ptree_.put_child(item.first, item.second);
        }
    }
