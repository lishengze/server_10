...
void CheckSPMC(std::vector<SPSCUnboundedQueue<uint64_t>*> mq_check_vec, uint64_t total_msgs)
{
    ...
    std::cout << "Check spmc success, totol receive <" << counter << ">, ";
    for (size_t i = 0; i < mq_size; i++)
    {
        std::cout << "< consumer-" << i << ", " << consumer_cnt[i] << ">, ";
    }
    std::cout << std::endl;
}
...
void CheckMPMC(std::vector<SPSCUnboundedQueue<QDate>*> mq_check_vec, uint64_t total_msgs)
{
    ...
    std::cout << "Check spmc success, totol receive <" << total_msgs << ">, ";
    for (size_t i = 0; i < mq_size; i++)
    {
        std::cout << "< consumer-" << i << ", 0:" << consumer_mp_cnt[0][i] << ", 1:" << consumer_mp_cnt[1][i] << ">, ";
    }
    std::cout << std::endl;
}
