...
void Producer(SPSCQueue<uint64_t>* p_to_c, uint64_t rate)
{
    while (!g_is_start);
...
void Consumer(SPSCQueue<uint64_t>* p_to_c)
{
    while (!g_is_start);
...
int main(int argc, char const *argv[])
{
    ...
    std::thread p_thread(std::bind(Producer, p_to_c1, atol(argv[2])));
    std::thread c_thread(std::bind(Consumer, p_to_c2));
