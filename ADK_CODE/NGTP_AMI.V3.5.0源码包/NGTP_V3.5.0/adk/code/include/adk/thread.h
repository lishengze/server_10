/**
 * @brief      implementation of adk thread model
 * @author     zhaonan, zhangwei
 * @date       20180127
 */
#ifndef ADK_IMPL_THREAD_H_
#define ADK_IMPL_THREAD_H_
 ...
 */
template<typename AppMessageType>
class ThreadMessageBase : public IObject, public GCRequest
{
public:
    /**
     * @brief      获取消息所属的分类
     ...
     * @param[in]  tag  消息分类标签
     * 
     * @note       ID取值范围为[0, ADK_THREAD_MAX_MESSAGE_TAG-1],目前仅支持128个分类
     */
...
public:
    ...
    // ThreadMessage(const ThreadMessage<AppMessageType>& other)
    // {
    //     message_base_ = other.message_base_;
    //     message_base_->hold_reference();
    // }

    // explicit ThreadMessage(const ThreadMessage<AppMessageType>&& other)
    // {
    //     message_base_ = other.message_base_;
    //     message_base_->hold_reference();
    // }
...
#define _ADK_DEFINE_THREAD_MESSAGE(AppThreadMessage, oob)     \
    template<typename T>    \
    class AppThreadMessage##Basic : public adk_impl::ThreadMessageBase<T>  \
    {   \
    public: \
        AppThreadMessage##Basic()   \
        {   \
            static int32_t s_message_type =     \
                    adk_impl::AllocMessageType(oob); \
            adk_impl::ThreadMessageBase<T>::message_type_ = s_message_type; \
            adk_impl::ThreadMessageBase<T>::message_type_name_ = #AppThreadMessage;  \
        }   \
    ...
    static adk_impl::OnceObject uniq_##AppThreadMessage([](){ \
        AppThreadMessage##Basic<AppThreadMessage>::New(adk_impl::gc::kDelayStart); \
    }); \
...
struct ThreadParams
{
    ...
    void Reset()
    {
        ...
        INIT_PARAM(event_mode, thread::kInterrupt);
...
public:
    ...
    { 
        return reinterpret_cast<ThreadClass*>(threads_[ThreadClass::Tag()][instance_id]);
    }
...
private:
    ThreadManager()
    {
        memset(threads_, 0x00, sizeof(threads_));
        nr_threads_ = 0;
    ...
    {
        return threads_[thr_tag][instance_id];
    }

    ThreadBase* threads_[ADK_MAX_THREAD_CLASS][ADK_MAX_THREAD_INSTANCE];
...
public:
                              ...
                              int32_t instance_id = 0)
    {
        ...
        new_thread->thread_message_queue_[ADK_MQ] = 
                        MPSCQueue::Create(new_thread->thread_name_,
                                          sizeof(void*),
                                          ADK_THREAD_MESSAGES_QUEUE_SIZE);

        new_thread->thread_message_queue_[ADK_OOB_MQ] = 
                        MPSCQueue::Create(std::string("OOB_") + new_thread->thread_name_,
                                          sizeof(void*),
                                          ADK_THREAD_OOB_MESSAGES_QUEUE_SIZE);

        new_thread->release_queue_ = SPSCQueue<SPSCUnboundedQueue<void*>**>::Create(
                                        "ReleaseThread",
                                        ADK_THREAD_MAX_MESSAGE_TAG * 2);

        new_thread->ev_mana_ = new SimpleEventManager(
                                        params->busy_poll_ns,
                                        params->backoff_limit);
...
protected:
    ...
    {
        SPSCUnboundedQueue<void*>* cache_queue;
        bool                       release_alert;
        uint32_t                   tag;
    ...
    SimpleEventManager* ev_mana_;
    MPSCQueue* thread_message_queue_[ADK_MQ_PER_THREAD];
    void*       msg_ptr_;
    ...
    // =============================================================
    SPSCUnboundedQueue<void*>** current_cache_queue_;
    SPSCQueue<SPSCUnboundedQueue<void*>**>* release_queue_;
...
public:
     ...
     *                              adk_impl::thread::Milliseconds(1000)
     ...
     *
     * @return     成功时返回Timer句柄，失败时返回 adk_impl::kInvalidTimerHandler
     */
    template<typename ThreadClass>
...
#define ADK_DEFINE_THREAD(ThreadClass, Desc)     \
    ...
    class ThreadClass##Basic : public adk_impl::ThreadBase  \
    {   \
    public: \
        ThreadClass##Basic()   \
        {   \
            s_thread_tag =     \
                    adk_impl::GenericThread::AllocThreadTag<ThreadClass##Fake>(); \
            thread_tag_ = s_thread_tag; \
            thread_name_ = #ThreadClass;  \
            thread_desc_ = Desc;    \
            message_handlers_ = adk_impl::GenericThread::AllocBuffer<ThreadClass##Fake>(); \
            RegisterMessageHandler<adk_impl::AsyncSignal>(   \
                            (adk_impl::GenericThread::HandlerType)(&adk_impl::ThreadBase::SignalHandler)); \
            RegisterMessageHandler<adk_impl::TimerSignal>(   \
                            (adk_impl::GenericThread::HandlerType)(&adk_impl::ThreadBase::TimerHandler)); \
        }   \
        \
        static int32_t Tag()    \
        {   \
        ...
        static void RegisterMessageHandler(const adk_impl::GenericThread::HandlerType hdl)    \
        {   \
            s_message_handlers = adk_impl::GenericThread::AllocBuffer<ThreadClass##Fake>();  \
    ...
    private:    \
        ...
        static adk_impl::GenericThread::HandlerType* s_message_handlers;  \
    };  \
    template<typename FakeType> \
    int32_t ThreadClass##Basic<FakeType>::s_thread_tag;  \
    template<typename FakeType> \
    adk_impl::GenericThread::HandlerType* ThreadClass##Basic<FakeType>::s_message_handlers;  \
    static adk_impl::OnceObject ADK_CONCATENATE(uniq_##ThreadClass, __LINE__)(   \
    ...
    // public: 
        ...
        // {   
            ...
            // ClassName##Basic<>::RegisterMessageHandler<MessageType>(  
                                    // (adk_impl::GenericThread::HandlerType)(mptr)); 
        // }   
    // } ADK_CONCATENATE(Uniq, __LINE__);
...
#define ADK_DEFINE_MESSAGE_HANDLER(...) ADK_DEFINE_MESSAGE_HANDLER_(ADK_GET_NARG(__VA_ARGS__), __VA_ARGS__)    \
                                        ...
                                        {   \
                                            ...
                                            this_type a = this; \
                                            typedef decltype(adk_impl::thread::GetClassType(a)) biz_gateway_class_type;   \
...
#define THREAD_DO_REGISTER_HANDLER(a, b)    \
            {   \
                ...
                RegisterMessageHandler<b>((adk_impl::GenericThread::HandlerType)(mptr));  \
            }
...
inline void ArgAssigment(ThreadParams* params, const GenericArg& arg)
{
    ...
    else if (arg == "MqContProcessLimit")
...
#define ADK_REGISTER_NS_THREAD_BEGIN(ns)   \
            static adk_impl::_RegisterHelper ADK_CONCATENATE(ns, __LINE__) = adk_impl::register_of(#ns)

/**
 * @brief      声明开始注册线程
 */
#define ADK_REGISTER_THREAD_BEGIN()   \
            static adk_impl::_RegisterHelper ADK_CONCATENATE(Default, __LINE__) = adk_impl::register_of("Default")
...
#define ADK_THREAD_CLASS(AppThread) \
            adk_impl::CreatorInfo([](adk_impl::ThreadManager* mana, adk_impl::ThreadParams* params,    \
                                int32_t id, adk_impl::ThreadParams** out_params)->adk_impl::ThreadBase* { \
                                    if (out_params != NULL) \
                                    {   \
                                    ...
                                    return adk_impl::ThreadBase::Create<AppThread>(mana, params, id);    \
                        },   \
                        #AppThread)
