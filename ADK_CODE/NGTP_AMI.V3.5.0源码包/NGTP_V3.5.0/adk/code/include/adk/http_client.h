#ifndef ADK_IMPL_HTTP_CLIENT_H_
#define ADK_IMPL_HTTP_CLIENT_H_
...
namespace http
{
    ...
    public:
        ...
        static void set_linger_to_zero(bool flag)
        {
            s_is_set_linger_to_zero = flag;
        }
    ...
    protected:
        ...
        std::string host;
        unsigned short port;
        static bool s_is_set_linger_to_zero;
        ...
        std::shared_ptr<boost::asio::steady_timer> get_timeout_timer(size_t timeout=0) {
            if(timeout==0)
    ...
    template<class socket_type>
    bool ClientBase<socket_type>::s_is_set_linger_to_zero = false;
                    ...
                    if(!ec) {
                        {
                            ...
                            if (s_is_set_linger_to_zero)
                            {
                                //typedef boost::asio::ip::tcp::socket HTTP;
                                auto socket_fd = socket->native_handle();
                                struct linger ling;
                                ling.l_linger = 0;
                                ling.l_onoff = 1;
                                //no care about failed
#if defined(__GNUC__)
                                setsockopt(socket_fd, SOL_SOCKET, SO_LINGER, (const void*)&ling, sizeof(ling));
#elif defined(_MSC_VER)
                                setsockopt(socket_fd, SOL_SOCKET, SO_LINGER, (const char*)&ling, sizeof(ling));
#endif
                            }
                        }
