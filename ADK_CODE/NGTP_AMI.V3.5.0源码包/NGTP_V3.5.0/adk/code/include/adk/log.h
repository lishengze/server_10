 ...
 * @date 2017-08-09
 */
#ifndef ADK_IMPL_LOG_H_
#define ADK_IMPL_LOG_H_
...
// 使用自定义日志实现
// 应用可继承adk_impl::Logger类实现自定义日志实现类。
// 如果未使用日志实现，日志将以默认格式输出到标准输出。
// @param logger 日志实现类指针
#define ADK_LOG_USE_LOGGER(logger) \
    do { \
        if (logger) { \
            adk_impl::log::g_logger = logger; \
            adk_impl::log::g_logger->set_min_log_level(adk_impl::log::g_log_min_level); \
        } \
    } while(false)
...
#define ADK_LOG_SET_THRESHOLD(level) \
    do { \
        adk_impl::log::g_log_min_level = static_cast<adk_impl::log::LogLevel>(level); \
        if (adk_impl::log::g_logger) \
            adk_impl::log::g_logger->set_min_log_level(adk_impl::log::g_log_min_level); \
    } while (false)
...
#define ADK_LOG_FORK() \
    do { \
        adk_impl::log::Logger::UpdatePidTid(); \
        if (adk_impl::log::g_logger) \
            adk_impl::log::g_logger->Fork(); \
    } while (false)
...
#define ADK_LOG_RAW(level, code, module_name, function, line, title, message) \
    do { \
        if (adk_impl::log::g_logger) {\
            if (adk_impl::log::g_logger->min_log_level() <= level) \
                adk_impl::log::g_logger->Log(level, code, module_name, function, line, \
                                        title, message); \
        } else \
            if (adk_impl::log::g_log_min_level <= level) \
                adk_impl::log::Logger::ConsoleLog(level, code, module_name, function, line, \
                                             title, message); \
    } while (false)

#define ADK_LOG_RAW_PID(pid, tid, level, code, module_name, function, line, title, message) \
    do { \
        if (adk_impl::log::g_logger) {\
            if (adk_impl::log::g_logger->min_log_level() <= level) \
                adk_impl::log::g_logger->Log(pid, tid, level, code, module_name, function, line, \
                                        title, message); \
        } else \
            if (adk_impl::log::g_log_min_level <= level) \
                adk_impl::log::Logger::ConsoleLog(pid, tid, level, code, module_name, function, line, \
                                             title, message); \
    } while (false)
...
#define ADK_LOG(level, code, title, message) \
    ADK_LOG_RAW(level, code, _module_name, __FUNCTION__, __LINE__, title, \
                adk_impl::log::_ToString(message))
#else
#define ADK_LOG(level, code, title, message) \
...
#define ADK_LOG_TF(level, code, title, ...) \
    ADK_LOG(level, code, adk_impl::log::_FormatLog(title), adk_impl::log::_FormatLog(__VA_ARGS__))

#define ADK_LOG_RAW_TF(level, code, module_name, function, line, title, message) \
    ADK_LOG_RAW(level, code, module_name, function, line, adk_impl::log::_FormatLog(title), \
                adk_impl::log::_FormatLog(message))
...
extern ADK_API Logger* g_logger;

extern ADK_API LogLevel g_log_min_level;
...
public:
    ...
    typedef ClockType::duration TimedurationType;

    /**
     * 默认间隔为1s打日志
     */
    explicit IntervalLogger(size_t seconds = 1)
            : interval_(InitValue(seconds))
    {}
...
private:
    TimedurationType InitValue(size_t interval) const
    {
        if (interval > 0)
        { return std::chrono::seconds(interval); }
        else
        { return TimedurationType::max(); }
    }

    TimepointType last_log_tp_;
    const TimedurationType interval_;
};
...
inline void LOG(adk_impl::log::SeverityLevel level,
                adk_impl::log::LogCode code,
